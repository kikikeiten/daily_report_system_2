...
<c:import url="/WEB-INF/views/layout/app.jsp">
    ...
    <c:param name="content">
                        ...
                        <c:forEach var="idea" items="${getManagerReviews}">
                            ...
                            <div class="ui green card">
                                <a class="content" href="<c:url value='/ideas/show?id=${idea.id}' />"> <span
                                        class="right floated"><fmt:formatDate value='${idea.idea_date}'
                                ...
                                <div class="extra content">
                                    ...
                                    <c:if test="${sessionScope.loginMember.id != idea.member.id && idea.member.role != 2}">
                                        <form method="POST" action="<c:url value='/reviews/manager/create' />"
                                              class="left floated">
                                            <input type="hidden" name="ideaId" value="${idea.id}"/>
                                            <button type="submit" name="reviewsStatus" value="${1}"
                                                    class="circular ui mini icon olive button">
                                    ...
                                    <a class="right floated date"
                                       href="<c:url value='/members/show?id=${idea.member.id}' />"> <c:out
                                            value="${idea.member.name}"/>
                                    </a>
                    ...
                    <div class="ui mini pagination menu">
                        <c:forEach var="i" begin="1" end="${((getManagerReviewsCnt - 1) / 12) + 1}" step="1">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a class="item" href="<c:url value='/reviews/manager?page=${i}' />"><c:out
                                            value="${i}"/></a>&nbsp;
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
        ...
        <a href="<c:url value='/' />" class="ui image label"> My ideas <span class="detail"> ${getMyIdeasCnt} </span>
        </a>

        <a href="<c:url value='/ideas' />" class="ui image label"> All ideas <span
                class="detail"> ${getIdeasCntButDrafts} </span>
        </a>
        ...
        <div class="ui teal image label">
            Manager's reviews
            <div class="detail">${getManagerReviewsCnt}</div>
        </div>

        <a href="<c:url value='/advice/director' />" class="ui image label"> Director's advice <span
