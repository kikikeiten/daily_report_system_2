    ...
    <c:param name="content">
        ...
        <c:choose>
            <c:when test="${idea != null}">
                <h2>日報「${idea.title}」の詳細ページ</h2>
                <table class="ui celled striped table">
                    <tbody>
                    <tr>
                        <th>氏名</th>
                        <td><c:out value="${idea.member.name}"/></td>
                    </tr>
                    <tr>
                        <th>フォロー</th>
                        <c:choose>
                    ...
                    <tr>
                        <th>承認状況</th>
                        <td><c:if test="${sessionScope.loginMember.role == 0 || sessionScope.loginMember.role == 1}">
                            <c:choose>
                                ...
                                <c:when test="${idea.reviewStatus == 1 && sessionScope.loginMember.id == idea.member.id}">
                                    <a href="<c:url value='/advice/manager' />">課長差し戻し</a>
                                </c:when>
                                <c:when test="${idea.reviewStatus == 1 && sessionScope.loginMember.id != idea.member.id}">
                                    課長差し戻し
                                </c:when>
                                ...
                                <c:when test="${idea.reviewStatus == 3 && sessionScope.loginMember.id == idea.member.id}">
                                    <a href="<c:url value='/advice/director' />">部長差し戻し</a>
                                </c:when>
                                <c:when
                                        test="${idea.reviewStatus == 3 && sessionScope.loginMember.id != idea.member.id}">
                                    部長差し戻し
                                </c:when>
                            ...
                            <c:if test="${sessionScope.loginMember.role == 2}">
                                <c:choose>
                                    ...
                                    <c:when test="${idea.reviewStatus == 2 && sessionScope.loginMember.id != idea.member.id}">
                                        <a href="<c:url value='/reviews/manager' />">課長承認待ち</a>
                                    </c:when>
                                    <c:when test="${idea.reviewStatus == 2 && sessionScope.loginMember.id == idea.member.id}">
                                        課長承認待ち
                                    </c:when>
                            ...
                            <c:if test="${sessionScope.loginMember.role == 3}">
                                <c:choose>
                                    ...
                                    <c:when test="${idea.reviewStatus == 4 && sessionScope.loginMember.id != idea.member.id}">
                                        <a href="<c:url value='/reviews/director' />">部長承認待ち</a>
                                    </c:when>
                                    <c:when test="${idea.reviewStatus == 4 && sessionScope.loginMember.id == idea.member.id}">
                                        部長承認待ち
                                    </c:when>
                    ...
                    <tr>
                        <th>承認履歴</th>
                        <td><a
                                href="<c:url value='/advice/history?id=${idea.id}' />">詳細を見る</a></td>
                    </tr>

                    <tr>
                        <th><c:out value="${name}"/><c:out value="${role}"/><c:out value="${reviewStatus}"/>コメント
                        </th>
                            ...
                            <c:choose>
                                <c:when test="${idea.reviewStatus == 0 && sessionScope.loginMember.id == idea.member.id}">
                                    <form method="POST" action="<c:url value='/drafts/update' />">
                                        ...
                                        <button type="submit" name="reviewStatus" value="${2}" class="ui positive button">提出
                                        </button>
                                    </form>
                                </c:when>
                                <c:when test="${idea.reviewStatus == 1 && sessionScope.loginMember.id == idea.member.id}">
                                    <form method="POST" action="<c:url value='/advice/manager/create' />">
                                        <input type="hidden" name="ideaId" value="${idea.id}"/>
                                        <button type="submit" name="reviewStatus" value="${2}" class="ui positive button">
                                            再提出
                                        </button>
                                    </form>
                                </c:when>
                                ...
                                <c:when test="${idea.reviewStatus == 3 && sessionScope.loginMember.id == idea.member.id}">
                                    <form method="POST" action="<c:url value='/advice/director/create' />">
                                        <input type="hidden" name="ideaId" value="${idea.id}"/>
                                        <button type="submit" name="reviewStatus" value="${4}" class="ui positive button">
                                            再提出
                                        </button>
                                    </form>
                                </c:when>
                                    ...
                                    <c:when test="${idea.reviewStatus == 2 && sessionScope.loginMember.id != idea.member.id && idea.member.role != 2}">
                                        <div style="display: inline-flex">
                                            <form method="POST" action="<c:url value='/reviews/manager/create' />" id="formId">
                                                ...
                                                &nbsp; <input type="hidden" name="ideaId" value="${idea.id}"/>
                                                <button type="submit" name="reviewStatus" value="${1}" class="ui negative button">
                                                    差し戻し
                                                </button>
                                            </form>
                                        </div>
                ...
                <c:choose>
                    <c:when
                            test="${sessionScope.loginMember.id != idea.member.id}">
                        <form method="POST" action="<c:url value='/favors/create' />">
                            ...
                            <div class="ui labeled button" tabindex="0">
                                ...
                                <c:if test="${idea.favors != 0}">
                                    <a href="<c:url value='/favors?id=${idea.id}' />" class="ui basic label"><c:out value="${idea.favors}"/></a>
                                </c:if>
                                <c:if test="${idea.favors == 0}">
